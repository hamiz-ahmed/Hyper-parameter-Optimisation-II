...
This is the code for Deep Learning Lab Exercise 6
"""
...
def create_config_space():
    ...
    config_space.add_hyperparameter(output_activation)

    optimizer = CS.CategoricalHyperparameter("optimizer", ["Adam", "SGD"], default_value='Adam')
    config_space.add_hyperparameter(optimizer)

    layers = CS.UniformIntegerHyperparameter("layers", lower=1,
                                             default_value=2, upper=4)
    config_space.add_hyperparameter(layers)

    num_units_0 = CS.UniformIntegerHyperparameter("num_units_0", lower=16,
                                                  default_value=32, upper=256, log=True)
    config_space.add_hyperparameter(num_units_0)

    num_units_1 = CS.UniformIntegerHyperparameter("num_units_1", lower=16,
                                                  default_value=32, upper=256, log=True)
    cond_num_units1 = CS.GreaterThanCondition(num_units_1, layers, 2)
    config_space.add_hyperparameter(num_units_1)
    config_space.add_condition(cond_num_units1)

    num_units_2 = CS.UniformIntegerHyperparameter("num_units_2", lower=16,
                                                  default_value=32, upper=256, log=True)
    cond_num_units2 = CS.GreaterThanCondition(num_units_2, layers, 3)
    config_space.add_hyperparameter(num_units_2)
    config_space.add_condition(cond_num_units2)

    num_units_3 = CS.UniformIntegerHyperparameter("num_units_3", lower=16,
                                                  default_value=32, upper=256, log=True)
    cond_num_units3 = CS.EqualsCondition(num_units_3, layers, 4)
    config_space.add_hyperparameter(num_units_3)
    config_space.add_condition(cond_num_units3)

    loss_function = CS.CategoricalHyperparameter("loss_function", ["categorical_crossentropy"],
                                                 default_value='categorical_crossentropy')
    config_space.add_hyperparameter(loss_function)
    ...
    config_space.add_hyperparameter(learning_rate_schedule)

    l2_reg_0 = CS.UniformFloatHyperparameter('l2_reg_0', lower=1e-6,
                                             upper=1e-2, default_value=1e-4, log=True)
    config_space.add_hyperparameter(l2_reg_0)

    l2_reg_1 = CS.UniformFloatHyperparameter('l2_reg_1', lower=1e-6,
                                             upper=1e-2, default_value=1e-4, log=True)
    cond_l2_reg1 = CS.GreaterThanCondition(l2_reg_1, layers, 2)
    config_space.add_hyperparameter(l2_reg_1)
    config_space.add_condition(cond_l2_reg1)

    l2_reg_2 = CS.UniformFloatHyperparameter('l2_reg_2', lower=1e-6,
                                             upper=1e-2, default_value=1e-4, log=True)
    cond_l2_reg2 = CS.GreaterThanCondition(l2_reg_2, layers, 3)
    config_space.add_hyperparameter(l2_reg_2)
    config_space.add_condition(cond_l2_reg2)

    l2_reg_3 = CS.UniformFloatHyperparameter('l2_reg_3', lower=1e-6,
                                             upper=1e-2, default_value=1e-4, log=True)
    cond_l2_reg3 = CS.EqualsCondition(l2_reg_3, layers, 4)
    config_space.add_hyperparameter(l2_reg_3)
    config_space.add_condition(cond_l2_reg3)

    dropout_0 = CS.UniformFloatHyperparameter('dropout_0', lower=0.0,
                                             upper=0.5, default_value=0.0, log=False)
    config_space.add_hyperparameter(dropout_0)

    dropout_1 = CS.UniformFloatHyperparameter('dropout_1', lower=0.0,
                                              upper=0.5, default_value=0.0, log=False)
    cond_dropout_1 = CS.GreaterThanCondition(dropout_1, layers, 2)
    config_space.add_hyperparameter(dropout_1)
    config_space.add_condition(cond_dropout_1)

    dropout_2 = CS.UniformFloatHyperparameter('dropout_2', lower=0.0,
                                              upper=0.5, default_value=0.0, log=False)
    cond_dropout_2 = CS.GreaterThanCondition(dropout_2, layers, 2)
    config_space.add_hyperparameter(dropout_2)
    config_space.add_condition(cond_dropout_2)

    dropout_3 = CS.UniformFloatHyperparameter('dropout_3', lower=0.0,
                                              upper=0.5, default_value=0.0, log=False)
    cond_dropout_3 = CS.EqualsCondition(dropout_3, layers, 4)
    config_space.add_hyperparameter(dropout_3)
    config_space.add_condition(cond_dropout_3)

    batch_size = CS.UniformIntegerHyperparameter("batch_size", lower=8,
                                                 default_value=16, upper=256, log=True)
    ...
    config_space.add_hyperparameter(activation)

    step_decay_epochs = CS.UniformIntegerHyperparameter("StepDecay epochs per step", lower=1,
                                                 default_value=16, upper=128, log=True)
    cond_step_decay_epochs = CS.EqualsCondition(step_decay_epochs, learning_rate_schedule, "StepDecay")
    config_space.add_hyperparameter(step_decay_epochs)
    config_space.add_condition(cond_step_decay_epochs)

    sgd_momentum = CS.UniformFloatHyperparameter('sgd_momentum', lower=0.0,
                                             upper=0.99, default_value=0.9, log=False)
    cond_sgd_momentum = CS.EqualsCondition(sgd_momentum, optimizer, "SGD")
    config_space.add_hyperparameter(sgd_momentum)
    config_space.add_condition(cond_sgd_momentum)

    sgd_initial_lr = CS.UniformFloatHyperparameter('sgd_initial_lr', lower=1e-3,
                                             upper=0.5, default_value=1e-1, log=True)
    cond_sgd_initial_lr = CS.EqualsCondition(sgd_initial_lr, optimizer, "SGD")
    config_space.add_hyperparameter(sgd_initial_lr)
    config_space.add_condition(cond_sgd_initial_lr)

    sgd_final_lr_fraction = CS.UniformFloatHyperparameter('sgd_final_lr_fraction', lower=1e-4,
                                             upper=1.0, default_value=1e-2, log=True)
    cond_sgd_final_lr_fraction = CS.EqualsCondition(sgd_final_lr_fraction, optimizer, "SGD")
    config_space.add_hyperparameter(sgd_final_lr_fraction)
    config_space.add_condition(cond_sgd_final_lr_fraction)


    adam_initial_lr = CS.UniformFloatHyperparameter('adam_initial_lr', lower=1e-4,
                                             upper=1e-2, default_value=1e-3, log=True)
    cond_adam_initial_lr = CS.EqualsCondition(adam_initial_lr, optimizer, "Adam")
    config_space.add_hyperparameter(adam_initial_lr)
    config_space.add_condition(cond_adam_initial_lr)

    adam_final_lr_fraction = CS.UniformFloatHyperparameter('adam_final_lr_fraction', lower=1e-4,
                                                    upper=1.0, default_value=1e-2, log=True)
    cond_adam_final_lr_fraction = CS.EqualsCondition(adam_final_lr_fraction, optimizer, "Adam")
    config_space.add_hyperparameter(adam_final_lr_fraction)
    config_space.add_condition(cond_adam_final_lr_fraction)
